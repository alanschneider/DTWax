...
__global__ void DTW(val_t   *subjects,
                    val_t   *query,
                    val_t   *dist,
                    index_t num_entries,
                    val_t   thresh)
{
  ...
  // Each thread computes CELLS_PER_THREAD adjacent cells, get corresponding
  // sig values
  //
...
__global__ void DTW(val_t *subjects,
  ...
  // Cooperative threading
  cg::thread_block_tile<GROUP_SIZE> g = cg::tiled_partition<GROUP_SIZE>(cg::this_thread_block());
  ...
  if ((thread_id == RESULT_THREAD_ID) && (REF_BATCH == 1)) {
    dist[block_id] = penalty_here[RESULT_REG] > thresh ? FLOAT2HALF(0) : FLOAT2HALF(1);
    return;
  }

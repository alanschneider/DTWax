...
int main(int argc, char **argv) {
  value_ht *host_query,                  // time series on CPU
           ...
           *device_last_row[STREAM_NUM]; // stores last column of sub-matrix
  ...
  // Target ref loading & re-organization for better mem coalescing & target
  // loading
  //
  ...
  ASSERT(cudaMallocManaged(&h_ref_coeffs_tmp,
                           (sizeof(reference_coefficients) * (REF_LEN)))); // host pinned memory for reference
  ...
  for (idxt l = 0; l < (REF_LEN / REF_TILE_SIZE); l += 1) {
    for (idxt i = 0; i < SEGMENT_SIZE; i++) {
      for (idxt j = 0; j < WARP_SIZE; j++) {
        h_ref_coeffs[k].coeff1 = h_ref_coeffs_tmp[(l * REF_TILE_SIZE) + (j * SEGMENT_SIZE) + i].coeff1;
        k++;
      }
    }
  }
  ...
  ASSERT(cudaMalloc(&(d_ref_coeffs), (sizeof(reference_coefficients) * REF_LEN)));

  ASSERT(cudaMemcpyAsync(d_ref_coeffs,
                         h_ref_coeffs, //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
  ...
  ASSERT(cudaMallocHost(&raw_array,
                        (sizeof(raw_t) * (NUM_READS * QUERY_LEN)))); // host pinned memory for raw data from FAST5
  ...
  // FLOAT to__half2
  //
  ASSERT(cudaMallocHost(&host_query, sizeof(value_ht) * (NUM_READS * QUERY_LEN + WARP_SIZE)));
  std::cout << "Normalized data:\n";

  for (index_t i = 0; i < NUM_READS; i++) {
    for (index_t j = 0; j < QUERY_LEN; j++) {
      host_query[(i * QUERY_LEN + j)] = FLOAT2HALF2(raw_array[(i * QUERY_LEN + j)]);
    }
  }
  ...
  for (int stream_id = 0; stream_id < STREAM_NUM; stream_id++) {
    ASSERT(cudaMalloc(&device_query[stream_id], (sizeof(value_ht) * (BLOCK_NUM * QUERY_LEN + WARP_SIZE))));
    ASSERT(cudaMalloc(&device_dist[stream_id],  (sizeof(value_ht) * BLOCK_NUM)));
    ASSERT(cudaStreamCreate(&stream_var[stream_id]));
    ASSERT(cudaMalloc(&device_last_row[stream_id], (sizeof(value_ht) * (REF_LEN * BLOCK_NUM))));
  }

  TIMERSTOP(malloc)
  cudaDeviceSetCacheConfig(cudaFuncCachePreferShared);
  
  // Mem I/O and DTW computation
  //
  ...
  for (idxt batch_id = 0; batch_id <= batch_count; batch_id += 1) {
    ...
    for (idxt stream_id = 1; (stream_id <= STREAM_NUM) && (rds_in_batch != 0);
      ...
      //----h2d copy-------------//
      ASSERT(cudaMemcpyAsync(device_query[stream_id - 1],
                             &host_query[(batch_id * STREAM_NUM * BLOCK_NUM * QUERY_LEN) + ((stream_id - 1) * BLOCK_NUM * QUERY_LEN)],
                             (sizeof(value_ht) * (QUERY_LEN * rds_in_stream + WARP_SIZE)),
                             cudaMemcpyHostToDevice,
                             stream_var[stream_id - 1]));

      //---------launch kernels------------//
      distances<value_ht, idxt>(d_ref_coeffs,
                                device_query[stream_id - 1],
                                device_dist[stream_id - 1],
                                rds_in_stream,
                                FLOAT2HALF2(0),
                                stream_var[stream_id - 1],
      ...
      //-----d2h copy--------------//
      ASSERT(cudaMemcpyAsync(&host_dist[(batch_id * STREAM_NUM * BLOCK_NUM) + ((stream_id - 1) * BLOCK_NUM)],
                             device_dist[stream_id - 1], 
                             (sizeof(value_ht) * rds_in_stream),
